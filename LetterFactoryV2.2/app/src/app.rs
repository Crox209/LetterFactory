    ...
    fn handle_dropped_files(&mut self, ctx: &egui::Context) {
        ...
        for f in dropped {
            ...
            let ext = path
                ...
                .unwrap_or_default();
            // While Bulk Mode is open, an .xlsx drop loads the Excel file there
            // (mirroring how a .docx drop loads the template on the main window).
            if self.bulk.open && ext == "xlsx" {
                self.bulk.load_dropped_excel(path);
                break;
            }
            if ext == "docx" {
                self.load_template(path);
